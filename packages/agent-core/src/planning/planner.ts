...
import { PLANNING_SYSTEM_PROMPT, buildStepExecutionSuffix } from './prompts.js';
 ...
 *
 * 规则：
 * - 消息长度 > 40 字 + 包含复杂度关键词 -> 需要
 * - 消息包含多个文件/模块提及 -> 需要
 * - 消息包含编号列表（用户已在拆步骤）-> 需要
 * - 消息包含简单关键词且长度短 -> 不需要
 * - 可选：通过 ProfileRegistry 判断模型是否受益于自动 Plan
 */
export function shouldPlan(
  message: string,
  options?: { modelId?: string; profileRegistry?: ProfileRegistry },
): boolean {
  // 检查模型是否受益于自动 Plan
  if (options?.modelId && options?.profileRegistry) {
    const profile = options.profileRegistry.resolve(options.modelId);
...
export interface GeneratePlanOptions {
  ...
  /** 用户消息 */
  userMessage: string;
  /** 项目上下文（可选，追加到 system prompt） */
  projectContext?: string;
  /** Profile 注册表（可选，用于读取模型参数） */
 ...
 */
export async function generatePlan(options: GeneratePlanOptions): Promise<Plan | null> {
  const { provider, model, userMessage, projectContext, profileRegistry } = options;
  ...
  // 从 ProfileRegistry 获取模型参数（如果可用）
  let maxTokens = 4096;
  let temperature = 0.3;
  let jsonMode = false;

  if (profileRegistry) {
    const profile = profileRegistry.resolve(model);
    const overrides = profile.overrides?.planning;
    maxTokens = overrides?.maxTokens ?? profile.defaults.maxTokens;
    temperature = overrides?.temperature ?? profile.defaults.temperature;
    jsonMode = profile.capabilities.jsonMode;
  }
